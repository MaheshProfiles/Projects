...
public class API_Get_AppointmentFor {
    private static final String MODULE = "API_Get_AppointmentFor";
    private static String TAG = "", Str_Msg = "";

    private String Str_Url = Constants_API.ApiGetAppointmentFor;
    private Activity mActivity;


    public API_Get_AppointmentFor(Activity mActivity) {
        this.mActivity = mActivity;
    }


    public void getAppointmentFor() {

        TAG = "getAppointmentFor";
        Log.d(MODULE, TAG);
    ...
    private void parseData(JSONArray response) {
        try {
            ArrayList<AppointmentFor> appointmentForArrayList = new ArrayList<>();
            if (response.length() > 0) {
                for (int i = 0; i < response.length(); i++) {
                    ...
                    AppointmentFor appointmentFor = new AppointmentFor(id, groupName, value, description);
                    appointmentForArrayList.add(appointmentFor);
                }
            }
            new Session(mActivity).saveAppointmentFor(appointmentForArrayList);
