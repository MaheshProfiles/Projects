...
public class API_Get_AppointmentStatus {
    private static final String MODULE = "API_Get_AS";
    private static String TAG = "", Str_Msg = "";

    private String Str_Url = Constants_API.ApiGet_AppointmentStatus;
    private Activity mActivity;


    public API_Get_AppointmentStatus(Activity mActivity) {
    ...
    private void parseData(JSONArray response) {
        try {
            ArrayList<AppointmentStatus> statusArrayList = new ArrayList<>();
            if (response.length() > 0) {
                for (int i = 0; i < response.length(); i++) {
                    ...
                    AppointmentStatus appointmentStatus = new AppointmentStatus(id, groupName, value, description);
                    statusArrayList.add(appointmentStatus);
                }
            }
            new Session(mActivity).saveAppointmentStatus(statusArrayList);
        } catch (Exception e) {
            e.printStackTrace();
        }
