...
public class API_Get_Gender {
    private static final String MODULE = "API_Get_Gender";
    private static String TAG = "", Str_Msg = "";

    private String Str_Url = Constants_API.ApiGetGender;
    private Activity mActivity;


    public API_Get_Gender(Activity mActivity) {
        ...
        try {
            ...
            if (response.length() > 0) {
                for (int i = 0; i < response.length(); i++) {
                    ...
                    String genderId="", genderCode="", description="";
                    genderId = jsonObject.getString("genderId");
                    genderCode = jsonObject.getString("genderCode");
                    description = jsonObject.getString("description");

                    Gender gender = new Gender(genderId, genderCode, description);
                    genderArrayList.add(gender);
                }
            }
            new Session(mActivity).savegender(genderArrayList);
