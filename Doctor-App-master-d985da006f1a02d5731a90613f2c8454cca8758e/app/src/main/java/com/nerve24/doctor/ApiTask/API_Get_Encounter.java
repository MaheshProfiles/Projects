...
public class API_Get_Encounter {
    private static final String MODULE = "API_Get_Encounter";
    private static String TAG = "", Str_Msg = "";

    private String Str_Url = Constants_API.ApiGetEncounter;
    private Activity mActivity;


    public API_Get_Encounter(Activity mActivity) {
        ...
        try {
            ...
            if (response.length() > 0) {
                for (int i = 0; i < response.length(); i++) {
                    ...
                    Encounter encounter = new Encounter(id, groupName, value, description);
                    encounterArrayList.add(encounter);
                }
            }
            new Session(mActivity).saveEncounter(encounterArrayList);
