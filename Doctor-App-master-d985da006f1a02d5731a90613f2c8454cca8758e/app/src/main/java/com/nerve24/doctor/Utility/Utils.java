...
public class Utils {
 ...
 /*   public static DisplayImageOptions getOptionsForRoundImg() {
    ...
    public static DailyWiseSlotDay parseData(JSONObject response, String clinicNameExist, String daySelected) {
        DailyWiseSlotDay dailyWiseSlotDay = null;
        try {
            // daySelected = daySelected.toLowerCase();
            JSONArray jsonArray = response.getJSONArray(daySelected);
            if (jsonArray.length() > 0) {
                for (int j = 0; j < jsonArray.length(); j++) {
                    ...
                    if (clinicNameExist.equals(clinicName)) {
                        dailyWiseSlotDay = new DailyWiseSlotDay(clinicName, clinicID, slotsId, startTime, endTime, day);
                        break;
                    }
                }
            }
