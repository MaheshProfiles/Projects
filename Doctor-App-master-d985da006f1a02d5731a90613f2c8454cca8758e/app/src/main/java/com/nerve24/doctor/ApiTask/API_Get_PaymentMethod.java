...
public class API_Get_PaymentMethod {
    private static final String MODULE = "API_Get_PaymentMethod";
    private static String TAG = "", Str_Msg = "";

    private String Str_Url = Constants_API.ApiPaymentMethods;
    private Activity mActivity;


    public API_Get_PaymentMethod(Activity mActivity) {
        ...
        try {
            ...
            if (response.length() > 0) {
                for (int i = 0; i < response.length(); i++) {
                    ...
                    PaymentMethod payObj = new PaymentMethod(paymentMethodId, paymentMethod);
                    paymentMethodArrayList.add(payObj);
                }
            }
            new Session(mActivity).savePaymentMethods(paymentMethodArrayList);
