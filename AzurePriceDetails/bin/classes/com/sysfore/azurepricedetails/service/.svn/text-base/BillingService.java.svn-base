package com.sysfore.azurepricedetails.service;
...
public class BillingService {
	...
	public Map<String, List<BillingPojo>> getBillDetails(String enrollment,
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (lineString.length > 10) {
					billingPojo.setSubscriptionName(lineString[1].replace("\"",""));
					...
					if (formatted.contains("00s")) {
						...
						if(lineString[20] != null ||(!lineString[20].equalsIgnoreCase("")))
							billingPojo.setComponent(lineString[20]);
						else
							billingPojo.setComponent(productFormatter);
						...
						if(lineString[19] != null ||(!lineString[19].equalsIgnoreCase("")))
					...
					// System.out.println(billingPojo.getSubscriptionName());

					List<BillingPojo> extractList = billingMap
							.get(lineString[4]);
	...
	public Map<String,List<BillingPojo>> getComponentBillDetails(String enrollment,
			String token, String month, String year) {
			...
			while((line = reader.readLine()) != null){ 
			   ...
			   if(lineString.length > 10) {
				   ...
				   if(formatted.contains("00s")) {
					   ...
					   billingPojo.setResourceQtyConsumed(formatted);
					   billingPojo.setServiceSubRegion(lineString[18]);
					   formatted = lineString[22].replace("\"", "");
					   if(formatted != null && (!formatted.equalsIgnoreCase("")))
						   billingPojo.setComponent(formatted);
					   else
						   billingPojo.setComponent(productFormatter);
					   Log.d("Myvalue22:", ":"+billingPojo.getComponent()+":");
					   billingPojo.setServiceInfo(lineString[21]);
				   }else{ 
					   billingPojo.setResourceQtyConsumed(formatted);
					   billingPojo.setServiceSubRegion(lineString[17]);
					   formatted = lineString[21].replace("\"", "");
					   if(formatted != null && (!formatted.equalsIgnoreCase("")))
						   billingPojo.setComponent(formatted);
					   else
						   billingPojo.setComponent(productFormatter);
					   Log.d("Myvalue21:", ":"+billingPojo.getComponent()+":");
					   billingPojo.setServiceInfo(lineString[20]);
				   }
				   
				   //System.out.println(billingPojo);
				   
				   
				   List<BillingPojo> extractList = billingMap.get(billingPojo.getComponent());
				   if(extractList != null && extractList.size() != 0) {
					   extractList.add(billingPojo);
					   ...
					   if(!lineString[4].equals("\"SubscriptionGuid\"")) {
						   extractList = new ArrayList<BillingPojo>();
						   extractList.add(billingPojo);
						   billingMap.put(billingPojo.getComponent(),extractList);
					   }
				   }
