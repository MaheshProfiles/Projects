...
public class API_Get_Salutation {
    private static final String MODULE = "API_Get_Salutation";
    private static String TAG = "", Str_Msg = "";

    private String Str_Url = Constants_API.ApiGetSalutation;
    private Activity mActivity;


    public API_Get_Salutation(Activity mActivity) {
    ...
    private void parseData(JSONArray response) {
        try {
            ArrayList<Salutation> salutationArrayList = new ArrayList<>();
            if (response.length() > 0) {
                for (int i = 0; i < response.length(); i++) {
                    JSONObject jsonObject = response.getJSONObject(i);
                    String salutation_id = "", salutation = "", description = "", isIndexed = "";
                    salutation_id = jsonObject.getString("salutation_id");
                    salutation = jsonObject.getString("salutation");
                    description = jsonObject.getString("description");
                    isIndexed = jsonObject.getString("isIndexed");

                    Salutation salutationObj = new Salutation(salutation_id, salutation, description, isIndexed);
                    salutationArrayList.add(salutationObj);
                }
            }
            new Session(mActivity).saveSalutation(salutationArrayList);
